// The included modules for arm_mover are the same as simple_arm, with the exception of 
// one new file. Namely, the GoToPosition.h header file, which is the header file 
// generated from the GoToPosition.srv file we created earlier.
...
// The clamp_at_boundaries() function is responsible for enforcing the minimum and maximum 
// joint angles for each joint. If the joint angles passed in are outside of the operable 
// range, they will be “clamped” to the nearest allowable value. The minimum and maximum 
// joint angles are retrieved from the parameter server each time clamp_at_boundaries 
// is called. The rest of this function simply clamps the joint angle if necessary. 
...
// When a client sends a GoToPosition request to the safe_move service, either from 
// the terminal or from a separate node the handle_safe_move_request function is called. 
// The function parameter req is of type GoToPosition::Request. And the service response 
// parameter res is of type GoToPosition::Response.
...
// The safe_move service will be blocked for 3 seconds so the arm has enough time to 
// move to the requested position.

    // Wait 3 seconds for arm to settle
    ros::Duration(3).sleep();

// Finally, the safe_move service returns back a message indicating that the arm has moved
// to its new position and displays the clamped joint angles.
...
// Inside the C++ main function, the arm_mover node is initialized and a ROS 
// NodeHandle object n is instantiated to communicate with ROS. 
...
// As we did earlier in the simple_arm node, two publisher objects are 
// created to publish joint angles to the arm. These objects are defined globally so as to 
// be easily accessible from all the other functions.
...
// Next, the GoToPosition service is created with the node name followed by safe_move.
// Generally, you want to name your services with the node name first to easily find 
// them in large projects. This service is defined with a handle_safe_move_request callback 
// function. The callback function runs when a service request is received.
